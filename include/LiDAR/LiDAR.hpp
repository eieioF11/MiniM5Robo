...
namespace LiDAR {
  ...
  private:
    ...
    typedef enum { STATE_WAIT_HEADER = 0, STATE_READ_HEADER, STATE_READ_PAYLOAD, STATE_READ_DONE } State_t;
    typedef struct {
      uint8_t header0;
      uint8_t header1;
      uint8_t header2;
    ...
    uint16_t convertDegree(uint16_t input) { return (input - 40960) / 64; }

    uint16_t convertSpeed(uint16_t input) { return input / 64; }
    ...
    void plot_point(point_t<double> p, uint32_t color) { M5.Display.drawPixel((uint32_t)(p.x * 100.0) + 160, (uint32_t)(p.y * 100.0) + 120, color); }
  ...
  public:
    ...
    void begin(uint8_t rx, uint8_t tx, const char* frame_id, bool visualize = true) {
      LiDAR_serial_.begin(230400, SERIAL_8N1, rx, tx);
      // LiDAR_serial_.begin(921600, SERIAL_8N1, rx, tx);
      visualize_                      = visualize;
      laser_scan_msg.header.frame_id  = micro_ros_string_utilities_set(laser_scan_msg.header.frame_id, frame_id);
    ...
    void set_visualize(bool visualize) { visualize_ = visualize; }
